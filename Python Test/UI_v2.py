...
class App:
	...
	def __init__(self, masterWindow, serialWindow):			################Initializer
		...
		#UI setup
		self.UILabel=tk.Label(self.masterWindow, text="Control")
		...
		#mode
		self.contFrame=tk.Frame(self.masterWindow)
		self.contFrame.grid(row=3, column=0, columnspan=2)
		self.contLabel=tk.Label(self.contFrame, text="Mode: ")
		self.contLabel.grid(row=0, column=0)
		self.contBtn=tk.Button(self.contFrame, text="Discontinous", command=self.set_mode)
		self.contBtn.grid(row=0, column=1)
		...
		for i in range(N_CHANNEL):	#initialization of channel array
			...
			#slider
			self.slider[i]=tk.Scale(self.channelFrame, from_=VAL_MAX, to=VAL_MIN, length=200, command=partial(self.read_field, i, 1)) # creates widget
	...
	def timer_call(self):									################Timed update function for Continous mode
		...
		#global chVal
		self.serialApp.test_connection()
	...
	def set_channel(self, x):								################communication done here
		...
		if self.serialApp.status()==1:
			if chVal[x]!=self.slider[x].get() :
				...
				par ^= par >> 1
				parity=(~par)
				c1= (((x&0x1F)<<3)|((val>>7)&0x7))&0xff
				c2= (val<<1|(parity&0x1))&0xff
	...
	def set_to_discontinous(self):							################ Force all to discontinous mode
		self.contBtn.config(text="Discontinous")
		self.mode=0
	...
	def set_mode(self):										################set Continous/Discontinous Mode
		...
		if self.mode==0 and self.serialApp.status()==1:	#set to Continous mode
			self.contBtn.config(text="  Continous   ")
			self.mode=1
			for i in range(N_CHANNEL):
			#	set_channel(i)
