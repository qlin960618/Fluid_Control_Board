...
chVal=[0 for x in range(N_CHANNEL)]
...
class App:
	...
	def __init__(self, masterWindow, serialWindow):			################Initializer
		...
		self.mode=1
		self.modeBtn=tk.Button(self.modeFrame, text="  Continous   ", command=self.set_mode)
		...
		#Variable Channel Control Array
		self.slider=[0 for x in range(N_CHANNEL)]
		self.sliderLabel=[0 for x in range(N_CHANNEL)]
		self.sliderSetBtn =[0 for x in range(N_CHANNEL)]
		self.sliderEntry=[0 for x in range(N_CHANNEL)]
		self.chValkPa=[0 for x in range(N_CHANNEL)]
		self.setMaxBtn=[0 for x in range(N_CHANNEL)]
		self.setMinBtn=[0 for x in range(N_CHANNEL)]
		self.chValEntryField=[0 for x in range(N_CHANNEL)]
		self.chStatusLabel=[0 for x in range(N_CHANNEL)]


		#additional Label
		self.slabel=tk.Label(self.channelFrame, text="Slider")
		self.slabel.grid(column=0, row=1)
		self.unitlabel=tk.Label(self.channelFrame, text="kPa")
		self.unitlabel.grid(column=0, row=2)
		self.elabel=tk.Label(self.channelFrame, text="Entry Field")
		self.elabel.grid(column=0, row=3)
		self.statusLabel=tk.Label(self.channelFrame, text="Status")
		self.statusLabel.grid(column=0, row=4)
		self.btnlabel=tk.Label(self.channelFrame, text="Update")
		self.btnlabel.grid(column=0, row=6)
		...
		for i in range(N_CHANNEL):	#initialization of channel array
			...
			#label in pressure 
			self.chValkPa[i]=tk.Label(self.channelFrame,text="%.2f"%0)
			self.chValkPa[i].grid(column=i*2+1, row=2, columnspan=2)
			...
			self.sliderEntry[i].grid(column=i*2+1, row=3, columnspan=2)
			
			#Status Label
			self.chStatusLabel[i]=tk.Label(self.channelFrame, text="  ",bg="green")
			self.chStatusLabel[i].grid(column=i*2+1, row=4, columnspan=2)

			#min max btn
			self.setMinBtn[i]=tk.Button(self.channelFrame, text="Min", command=partial(self.set_mn, i, 0))
			self.setMinBtn[i].grid(column=i*2+1, row=5)
			self.setMaxBtn[i]=tk.Button(self.channelFrame, text="Max", command=partial(self.set_mn, i, 1))
			self.setMaxBtn[i].grid(column=(i+1)*2, row=5)

			#Set Button
			self.sliderSetBtn[i]=tk.Button(self.channelFrame, text="Set" , command=partial(self.set_channel, i), state='disabled')
			self.sliderSetBtn[i].grid(column=i*2+1, row=6, columnspan=2)
	...
	def read_field(self, x, src, dc):  						################src define source 0: entry, 1: slider 
		...
		if src == 0:	#source from entyr field
			if VERBOSE_LEVEL>1:
		...
		elif src == 1:	#sourced from slider
			if VERBOSE_LEVEL>1:
		...
		if SENSOR_MIX:
			self.chValkPa[x].config(text="%.2f"%(self.slider[x].get()*SENSOR_RANGE[x]*0.007013712565))
		else:
			self.chValkPa[x].config(text="%.2f"%(self.slider[x].get()*SENSOR_RANGE*0.007013712565))
	...
	def set_channel(self, x):								################communication done here
		...
		if self.serialApp.status()==1:
			...
			if chVal[x]!=val:
				...
				self.serialApp.reset_buffer() #reset Read Buffer
				self.serialApp.sent_byte(b's') #set command type
				self.serialApp.sent_bytes(c0, c1) #sent data bytes
				b=0
				b=self.serialApp.read_bytes()


				if b==b'k' and self.serialApp.status()==1:	#return String match
					print("Channel %d is set to %d"%(x+1, val))
					chVal[x]=val
					self.updateRequired=True
					self.chStatusLabel[x].config(bg="red")  #Update channel status
					self.update_label(x)
				else:	#return string missmatch
	...
	def request_channel_status(self):
		...
		if self.serialApp.status()==1:
			...
			self.serialApp.sent_byte(b'p') #set command type

			c0 = self.serialApp.read_bytes() #read status
			c1 = self.serialApp.read_bytes()
			#print(c0)
			#print(c1)
			c=(ord(c0)<<8)|ord(c1)
			#print(bin(c))
			...
			for i in reversed(range(N_CHANNEL)):
				if (c&0x1)==0:
					self.chStatusLabel[i].config(bg="green")
				else:
					self.chStatusLabel[i].config(bg="red")
				c=c>>1
	...
	def set_mode(self):										################toggle Continous/Discontinous Mode
		if VERBOSE_LEVEL>0:
